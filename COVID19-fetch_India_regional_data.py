...
rows = 27	#The number of rows to be fetched from the table (this may change).
...
updated_tally = tables[-1][:rows].drop(["S. No.", "Total Confirmed cases ( Foreign National )"], axis = 1)
updated_tally = updated_tally.rename(columns = {"Name of State / UT": "Region", "Total Confirmed cases (Indian National)": "Confirmed", "Cured/Discharged/Migrated": "Recovered/Migrated", "Death": "Deceased"})
updated_tally = updated_tally.astype({"Confirmed": int, "Recovered/Migrated": int, "Deceased": int})
updated_tally.loc[:, "Confirmed"] = updated_tally["Confirmed"] + tables[-1][:rows]["Total Confirmed cases ( Foreign National )"].astype(int)
